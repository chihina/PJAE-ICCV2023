...
for epoch_num in range(5, 100, 5):
# for epoch_num in range(100, 105, 5):
    model_head_weight_path = os.path.join(weight_saved_dir, f"model_head_epoch_{epoch_num}.pth")
    ...
    if 'bbox_PRED_gaze_PRED_act_PRED' in model_name and cfg.model_params.model_type == 'ja_transformer_dual':
        ...
        model_saliency_weight_path = os.path.join(os.path.join(cfg.exp_set.save_folder,cfg.data.name, model_name), f"model_saliency_epoch_{epoch_num}.pth")

    model_saliency_weight_path = os.path.join(os.path.join(cfg.exp_set.save_folder,cfg.data.name, model_name), f"model_saliency_epoch_{epoch_num}.pth")
    model_saliency.load_state_dict(torch.load(model_saliency_weight_path,  map_location='cuda:'+str(gpus_list[0])))

    model_attention_weight_path = os.path.join(weight_saved_dir, f"model_gaussian_epoch_{epoch_num}.pth")
    model_attention.load_state_dict(torch.load(model_attention_weight_path,  map_location='cuda:'+str(gpus_list[0])))

    model_fusion_weight_path = os.path.join(weight_saved_dir, f"model_fusion_epoch_{epoch_num}.pth")
    ...
    save_results_dir = os.path.join('results', cfg.data.name, model_name, 'eval_results', data_type_id, f'epoch_{epoch_num}')
