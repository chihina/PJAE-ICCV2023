...
l2_dist_array = np.zeros((len(test_data_loader), 1, 10))
pred_acc_array = np.zeros((len(test_data_loader), 255, 2))
...
for iteration, batch in enumerate(test_data_loader):
    ...
    final_map_peak = np.max(final_joint_attention_heatmap)
    for peak_thresh_idx in range(255):
        peak_thresh = peak_thresh_idx/255
        co_att_flag_pred = final_map_peak > peak_thresh
        pred_acc_array[iteration, peak_thresh_idx, :] = [co_att_flag_gt, co_att_flag_pred]
    ...
    l2_dist_array[iteration, 0, :] = np.array(l2_dist_list_append)
...
best_f1 = 0
best_peak_thresh_idx = -100
for peak_thresh_idx in range(255):
    co_att_gt_array = pred_acc_array[:, peak_thresh_idx, 0]
    co_att_pred_array = pred_acc_array[:, peak_thresh_idx, 1]
    ...
    if f1 > best_f1:
        best_f1 = f1
        best_peak_thresh_idx = peak_thresh_idx
        print('===Update best results===')
        print(f'Thr:{best_peak_thresh_idx}, Acc:{acc:.2f}, Prec:{prec:.2f}, Recall:{recall:.2f}, F1:{f1:.2f}')