...
class ModelSpatioTemporal(nn.Module):
    ...
    def forward(self, inp):
        ...
        head_bbox_flat = head_bbox.view(batch_size*frame_num*people_num, 4)
        head = torch.zeros(batch_size*frame_num*people_num, 1, resousion_height, resousion_width, device=images.device)
        for head_idx in range(batch_size*frame_num*people_num):
            x_min, y_min, x_max, y_max = head_bbox_flat[head_idx, :]
            x_min, x_max = x_min*resousion_width, x_max*resousion_width
            y_min, y_max = y_min*resousion_height, y_max*resousion_height
            x_min = x_min.long()
        ...
        face = F.interpolate(face, (resousion_height, resousion_width), mode='bilinear')
        images = images.view(batch_size*frame_num, 3, image_height, image_width)
        images = F.interpolate(images, (resousion_height, resousion_width), mode='bilinear')
        images = images.repeat(people_num, 1, 1, 1)
        ...
        # encoding = encoding.contiguous().view(frame_num, batch_size, people_num, 512, 7, 7)
        # hidden_scene = (torch.zeros((1, batch_size, 512, 7, 7), device=images.device),
            #   torch.zeros((1, batch_size, 512, 7, 7), device=images.device))
        # y_list = []
        # for t in range(frame_num):
            # y, hx = self.convlstm_scene(encoding[t], hx=hidden_scene)
            # hidden_scene = hx
            # y_list.append(y)
            # print(y.shape)
        # print(len(y_list))
        ...
        # deconv = deconv.contiguous().view(batch_size*frame_num*people_num, 512, 7, 7)
        
        # version2
        # encoding = encoding.view(batch_size, frame_num, people_num, 512, 7, 7)
        # encoding = encoding.permute(0, 2, 1, 3, 4, 5)
        # encoding = encoding.contiguous().view(batch_size*people_num, frame_num, 512, 7, 7)
        # hx=None
        # y, hx = self.convlstm_scene(encoding, hx=hx)
        # deconv = y.view(batch_size, people_num, frame_num, 512, 7, 7)
        # deconv = deconv.permute(0, 2, 1, 3, 4, 5)
        # deconv = deconv.contiguous().view(batch_size*frame_num*people_num, 512, 7, 7)
        ...
        out = {}
        out['encoding_inout'] = encoding_inout
        out['person_scene_attention_heatmap'] = x.view(batch_size, frame_num, people_num, 64, 64)
...
class ModelSpatialDummy(nn.Module):
    ...
    def forward(self, inp):
        ...
        rgb_img = inp['rgb_img']
        if len(rgb_img.shape) == 5:
            _, _, _, ori_height, ori_width = rgb_img.shape
        else:
