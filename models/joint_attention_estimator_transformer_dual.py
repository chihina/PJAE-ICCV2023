...
class JointAttentionEstimatorTransformerDual(nn.Module):
    def __init__(self, cfg):
        ...
        if self.gaze_map_estimator_type == 'identity':
            self.gaze_map_estimator_people = nn.Sequential(
                nn.Linear(self.people_feat_dim, estimate_param_num),
            )
            self.gaze_map_estimator_rgb = nn.Sequential(
                nn.Linear(self.people_feat_dim, estimate_param_num),
            )
            self.gaze_map_estimator_concat = nn.Sequential(
                nn.Linear(self.people_feat_dim*2, estimate_param_num),
            )
    ...
    def forward(self, inp):
        ...
        if self.head_embedding_type == 'each':
            ...
            if self.use_position and self.use_gaze and self.use_action:
                if self.use_position_enc_person:
                    head_info_params_emb = head_gaze_emb+head_action_emb
                else:
                    head_info_params_emb = head_position_emb+head_action_emb+head_gaze_emb
            elif self.use_position and self.use_gaze:
                if self.use_position_enc_person:
                    head_info_params_emb = head_gaze_emb
                else:
                    head_info_params_emb = head_gaze_emb+head_position_emb
            elif self.use_position and self.use_action:
                if self.use_position_enc_person:
                    head_info_params_emb = head_action_emb
                else:
                    head_info_params_emb = head_action_emb+head_position_emb
            elif self.use_gaze and self.use_action:
                head_info_params_emb = head_action_emb+head_gaze_emb
            elif self.use_position:
                head_info_params_emb = head_position_emb
            elif self.use_gaze:
                head_info_params_emb = head_gaze_emb
            elif self.use_action:
                head_info_params_emb = head_action_emb
            ...
            head_info_params_emb_rgb = self.head_info_feat_embeding(head_info_params)
        ...
        if self.use_position and self.use_position_enc_person:
            ...
            head_info_params_emb = head_info_params_emb + head_position_encoding
        ...
        if self.rgb_cnn_extractor_type != 'no_use':
            ...
            head_info_params_emb_rgb_view = head_info_params_emb_rgb.view(self.batch_size, people_num, 1, self.people_feat_dim)
            ...
            rgb_people_feat_all_pos = torch.cat([rgb_feat_patch_pos_expand, head_info_params_emb_rgb_view], dim=-2)
            rgb_people_feat_all = torch.cat([rgb_feat_patch_expand, head_info_params_emb_rgb_view], dim=-2)
            ...
            rgb_people_feat_all = rgb_people_feat_all.view(self.batch_size, people_num, self.rgb_embeding_dim)
        else:
            # dummy
            ...
            rgb_people_feat_all = head_info_params_emb_rgb

        # estimate gaze map parameters
        head_vector_params_people = self.gaze_map_estimator_people(head_info_params_emb)
        head_vector_params_rgb = self.gaze_map_estimator_rgb(rgb_people_feat_all)
        extract_feat_concat = torch.cat([head_info_params_emb, rgb_people_feat_all], dim=-1)
        head_vector_params_concat = self.gaze_map_estimator_concat(extract_feat_concat)
        head_vector_params_people = head_vector_params_people.view(self.batch_size, people_num, self.dynamic_gaussian_num, self.estimate_param_num_base)
        head_vector_params_rgb = head_vector_params_rgb.view(self.batch_size, people_num, self.dynamic_gaussian_num, self.estimate_param_num_base)
        head_vector_params_concat = head_vector_params_concat.view(self.batch_size, people_num, self.dynamic_gaussian_num, self.estimate_param_num_base)
        head_vector_params_people[:, :, :, 3] = torch.exp(head_vector_params_people[:, :, :, 3].clone())
        head_vector_params_rgb[:, :, :, 3] = torch.exp(head_vector_params_rgb[:, :, :, 3].clone())
        head_vector_params_concat[:, :, :, 3] = torch.exp(head_vector_params_concat[:, :, :, 3].clone())
        ...
        # generate gaze map
        distance_mean_x_people = head_vector_params_people[:, :, :, None, None, None, 1]
        distance_mean_y_people = head_vector_params_people[:, :, :, None, None, None, 2]
        distance_sigma_people = head_vector_params_people[:, :, :, None, None, None, 3]
        distance_mean_x_rgb = head_vector_params_rgb[:, :, :, None, None, None, 1]
        distance_mean_y_rgb = head_vector_params_rgb[:, :, :, None, None, None, 2]
        distance_sigma_rgb = head_vector_params_rgb[:, :, :, None, None, None, 3]
        distance_mean_x_concat = head_vector_params_concat[:, :, :, None, None, None, 1]
        distance_mean_y_concat = head_vector_params_concat[:, :, :, None, None, None, 2]
        distance_sigma_concat = head_vector_params_concat[:, :, :, None, None, None, 3]

        distance_dist_x_people = (xy_axis_map[:, :, None, 0, :, :]-distance_mean_x_people[:, :, :, 0, :, :]) ** 2
        distance_dist_y_people = (xy_axis_map[:, :, None, 1, :, :]-distance_mean_y_people[:, :, :, 0, :, :]) ** 2
        distance_dist_denom_people = distance_sigma_people[:, :, :, 0, :, :]
        distance_dist_all_people = torch.exp(-(distance_dist_x_people+distance_dist_y_people) / (2 * distance_dist_denom_people))
        distance_dist_people = torch.sum(distance_dist_all_people, dim=2)

        distance_dist_x_rgb = (xy_axis_map[:, :, None, 0, :, :]-distance_mean_x_rgb[:, :, :, 0, :, :]) ** 2
        distance_dist_y_rgb = (xy_axis_map[:, :, None, 1, :, :]-distance_mean_y_rgb[:, :, :, 0, :, :]) ** 2
        distance_dist_denom_rgb = distance_sigma_rgb[:, :, :, 0, :, :]
        distance_dist_all_rgb = torch.exp(-(distance_dist_x_rgb+distance_dist_y_rgb) / (2 * distance_dist_denom_rgb))
        distance_dist_rgb = torch.sum(distance_dist_all_rgb, dim=2)

        distance_dist_x_concat = (xy_axis_map[:, :, None, 0, :, :]-distance_mean_x_concat[:, :, :, 0, :, :]) ** 2
        distance_dist_y_concat = (xy_axis_map[:, :, None, 1, :, :]-distance_mean_y_concat[:, :, :, 0, :, :]) ** 2
        distance_dist_denom_concat = distance_sigma_concat[:, :, :, 0, :, :]
        distance_dist_all_concat = torch.exp(-(distance_dist_x_concat+distance_dist_y_concat) / (2 * distance_dist_denom_concat))
        distance_dist_concat = torch.sum(distance_dist_all_concat, dim=2)

        if self.wandb_name == 'debug':
            for gauss_idx in range(self.dynamic_gaussian_num):
                print(f'{distance_dist_x_concat[0, 0, gauss_idx, 0, 0].item():.2f}', end=' ')
                print(f'{distance_dist_y_concat[0, 0, gauss_idx, 0, 0].item():.2f}')

        # multiply zero to padding maps
        distance_dist = distance_dist_concat
        ...
        head_tensor_people = torch.cat([head_vector, head_vector_params_people], dim=2)
        head_tensor_rgb = torch.cat([head_vector, head_vector_params_rgb], dim=2)
        head_tensor_concat = torch.cat([head_vector, head_vector_params_concat], dim=2)
        ...
        data['head_tensor_people'] = head_tensor_people
        data['head_tensor_rgb'] = head_tensor_rgb
        data['head_tensor_concat'] = head_tensor_concat
        data['head_tensor'] = head_tensor_concat
    ...
    def calc_loss(self, inp, out, cfg):
        ...
        head_tensor_people = out['head_tensor_people']
        head_tensor_rgb = out['head_tensor_rgb']
        head_tensor_concat = out['head_tensor_concat']
        ...
        distance_mean_xy_people = head_tensor_people[:, :, 3:5]
        distance_mean_xy_rgb = head_tensor_rgb[:, :, 3:5]
        distance_mean_xy_concat = head_tensor_concat[:, :, 3:5]
        loss_regress_xy_people = torch.pow(gt_box_xy_mid-distance_mean_xy_people, 2)
        loss_regress_xy_rgb = torch.pow(gt_box_xy_mid-distance_mean_xy_rgb, 2)
        loss_regress_xy_concat = torch.pow(gt_box_xy_mid-distance_mean_xy_concat, 2)
        loss_regress_xy = (loss_regress_xy_people+loss_regress_xy_rgb+loss_regress_xy_concat)/3
        ...
        loss_regress_xy_people_not_att = ((exploit_gt_box-distance_mean_xy_people) ** 2)
        loss_regress_xy_rgb_not_att = ((exploit_gt_box-distance_mean_xy_rgb) ** 2)
        loss_regress_xy_concat_not_att = ((exploit_gt_box-distance_mean_xy_concat) ** 2)
        loss_regress_xy_not_att = (loss_regress_xy_people_not_att+loss_regress_xy_rgb_not_att+loss_regress_xy_concat_not_att)/3
